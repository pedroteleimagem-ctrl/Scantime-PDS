...
if __name__ == '__main__':
    ...
    def open_forbidden_associations_dialog():
        posts = list(work_posts)
        working_pairs = set(Assignation.FORBIDDEN_POST_ASSOCIATIONS)
        enable_var = tk.BooleanVar(value=different_post_var.get())
        posts_bind_id = None

        dialog = tk.Toplevel(root)
        dialog.title("Associations Interdites")
        dialog.transient(root)
        dialog.resizable(False, False)
        dialog.grab_set()
        ...
        enable_check = ttk.Checkbutton(
            content,
            text="Activer les associations interdites",
            variable=enable_var
        )
        enable_check.grid(row=0, column=0, sticky="w")

        morning_var = tk.StringVar()
        afternoon_var = tk.StringVar()
        display_pairs = []

        form_frame = ttk.Frame(content)
        form_frame.grid(row=1, column=0, pady=(10, 6), sticky="ew")
        form_frame.columnconfigure(1, weight=1)
        form_frame.columnconfigure(3, weight=1)

        ttk.Label(form_frame, text="Poste matin :").grid(row=0, column=0, sticky="w")
        morning_combo = ttk.Combobox(form_frame, textvariable=morning_var, state="readonly", values=posts)
        morning_combo.grid(row=0, column=1, sticky="ew", padx=(6, 12))

        ttk.Label(form_frame, text="Poste apres-midi :").grid(row=0, column=2, sticky="w")
        afternoon_combo = ttk.Combobox(form_frame, textvariable=afternoon_var, state="readonly", values=posts)
        afternoon_combo.grid(row=0, column=3, sticky="ew", padx=(6, 0))

        list_frame = ttk.Frame(content)
        list_frame.grid(row=2, column=0, sticky="nsew")
        content.rowconfigure(2, weight=1)

        pairs_list = tk.Listbox(list_frame, height=8, selectmode="extended")
        pairs_list.pack(side="left", fill="both", expand=True)
        list_scroll = ttk.Scrollbar(list_frame, orient="vertical", command=pairs_list.yview)
        list_scroll.pack(side="right", fill="y")
        pairs_list.configure(yscrollcommand=list_scroll.set)

        actions_frame = ttk.Frame(content)
        actions_frame.grid(row=3, column=0, pady=(6, 0), sticky="ew")

        def refresh_pairs_list():
            nonlocal display_pairs
            valid_posts = set(posts)
            if any((m not in valid_posts or a not in valid_posts) for m, a in working_pairs):
                working_pairs.intersection_update({pair for pair in working_pairs if pair[0] in valid_posts and pair[1] in valid_posts})
            display_pairs = sorted(working_pairs)
            pairs_list.delete(0, "end")
            for morning_name, afternoon_name in display_pairs:
                pairs_list.insert("end", f"{morning_name} -> {afternoon_name}")

        def handle_posts_update(_event=None):
            nonlocal posts
            posts = list(work_posts)
            morning_combo["values"] = posts
            afternoon_combo["values"] = posts
            if morning_var.get() not in posts:
                morning_var.set("")
            if afternoon_var.get() not in posts:
                afternoon_var.set("")
            refresh_pairs_list()

        def add_pair():
            morning = morning_var.get().strip()
            afternoon = afternoon_var.get().strip()
            if not morning or not afternoon:
                messagebox.showwarning("Associations interdites", "Veuillez choisir un poste matin et un poste apres-midi.")
                return
            pair = (morning, afternoon)
            if pair in working_pairs:
                messagebox.showinfo("Associations interdites", "Cette association est deja interdite.")
                return
            working_pairs.add(pair)
            refresh_pairs_list()

        def remove_selected():
            indices = list(pairs_list.curselection())
            if not indices:
                return
            for idx in reversed(indices):
                if 0 <= idx < len(display_pairs):
                    working_pairs.discard(display_pairs[idx])
            refresh_pairs_list()
        ...
        def seed_same_post():
            ...
            for post in posts:
                pair = (post, post)
                if pair not in working_pairs:
                    working_pairs.add(pair)
                    added = True
            if not added:
                messagebox.showinfo("Associations interdites", "Les memes postes sont deja interdits.")
            refresh_pairs_list()

        def clear_pairs():
            if not working_pairs:
                return
            if not messagebox.askyesno("Associations interdites", "Supprimer toutes les associations interdites ?"):
                return
            working_pairs.clear()
            refresh_pairs_list()

        def close_dialog():
            nonlocal posts_bind_id
            if posts_bind_id is not None:
                try:
                    root.unbind("<<WorkPostsUpdated>>", posts_bind_id)
                except Exception:
                    pass
                posts_bind_id = None
            try:
        ...
        def apply_changes():
            try:
                valid_posts = set(work_posts)
                sanitized = {(m, a) for (m, a) in working_pairs if m in valid_posts and a in valid_posts}
                Assignation.ENABLE_DIFFERENT_POST_PER_DAY = bool(enable_var.get())
                different_post_var.set(Assignation.ENABLE_DIFFERENT_POST_PER_DAY)
                Assignation.FORBIDDEN_POST_ASSOCIATIONS.clear()
                Assignation.FORBIDDEN_POST_ASSOCIATIONS.update(sanitized)
                for gui_instance, _, _ in tabs_data:
                    try:
                        gui_instance.schedule_update_colors()
                    except Exception:
                        pass
        ...
        ttk.Button(form_frame, text="Ajouter", command=add_pair).grid(row=0, column=4, padx=(12, 0))
        ttk.Button(actions_frame, text="Supprimer selection", command=remove_selected).pack(side="left")
        ttk.Button(actions_frame, text="Interdire meme poste", command=seed_same_post).pack(side="left", padx=(8, 0))
        ttk.Button(actions_frame, text="Tout effacer", command=clear_pairs).pack(side="left", padx=(8, 0))

        footer = ttk.Frame(content)
        footer.grid(row=4, column=0, pady=(12, 0), sticky="e")
        ttk.Button(footer, text="Annuler", command=close_dialog).pack(side="right", padx=(6, 0))
        ttk.Button(footer, text="Valider", command=apply_changes).pack(side="right")

        posts_bind_id = root.bind("<<WorkPostsUpdated>>", handle_posts_update, add="+")
        dialog.bind("<Escape>", lambda _e: close_dialog())
        dialog.protocol("WM_DELETE_WINDOW", close_dialog)
        refresh_pairs_list()
        dialog.focus_set()

    def toggle_different_post():
        open_forbidden_associations_dialog()

    def toggle_limitation():
        Assignation.ENABLE_MAX_ASSIGNMENTS = limitation_enabled_var.get()
        state_str = "activée" if limitation_enabled_var.get() else "désactivée"
        messagebox.showinfo("Configuration", f"Limitation d'affectation {state_str}.")

    def config_max_assignments():
        # On rÃ©cupÃ¨re la position actuelle du pointeur de souris
        x = root.winfo_pointerx()
        y = root.winfo_pointery()
        # CrÃ©ation d'un Toplevel placÃ© Ã  cÃ´tÃ© du clic
        dialog = tk.Toplevel(root)
        dialog.title("Configuration")
        dialog.geometry(f"+{x}+{y}")
        # Texte et Spinbox pour le nombre
        tk.Label(dialog, text="Entrez le nombre maximum de postes par semaine :", 
                 font=(APP_FONT_FAMILY, 10)).pack(padx=10, pady=(10, 0))
        spin = tk.Spinbox(dialog, from_=1, to=100, width=5, font=(APP_FONT_FAMILY, 10))
        spin.delete(0, 'end')
        spin.insert(0, Assignation.MAX_ASSIGNMENTS_PER_POST)
        ...
        def on_ok():
            ...
            result['new'] = val
            dialog.destroy()
        tk.Button(dialog, text="OK", command=on_ok, font=(APP_FONT_FAMILY, 10)).pack(pady=(0, 10))
        # Modal et attente de fermeture
        dialog.transient(root)
        dialog.grab_set()
        root.wait_window(dialog)
        # Application du nouveau paramÃ¨tre si valide
        if 'new' in result:
            Assignation.MAX_ASSIGNMENTS_PER_POST = result['new']
            messagebox.showinfo("Configuration", 
                                f"Nombre maximum modifié : {result['new']} affectations par poste.")


    def toggle_repos_securite():
        Assignation.ENABLE_REPOS_SECURITE = repos_securite_var.get()
        state_str = "activé" if repos_securite_var.get() else "désactivé"
        messagebox.showinfo("Configuration", f"Repos de sécurité {state_str}.")

    setup_menu.add_command(
        label="Associations Interdites",
        command=open_forbidden_associations_dialog
    )
    setup_menu.add_checkbutton(
        label="Activer limitation d'affectation",
        variable=limitation_enabled_var,
        command=toggle_limitation
    )
    setup_menu.add_command(label="Configurer nombre maximum", command=config_max_assignments)
    setup_menu.add_checkbutton(
        label="Activer repos de sécurité",
        variable=repos_securite_var,
        command=toggle_repos_securite
    )
